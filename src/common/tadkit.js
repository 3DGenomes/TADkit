...
var TADkit = angular.module('TADkit',['ngRoute', 'mm.foundation']);
	...
	TADkit.controller('DashboardCtrl',['$scope', 'Assembly', function($scope, Assembly){
	...
	TADkit.controller('SceneCtrl',['$scope', '$timeout', 'TAD', 'Genes',
	function($scope, $timeout, TAD, Genes){
		// Timeout as temp fix to pause rendering until DOM complete
		$timeout( function() {$scope.vertices = TAD.getVertices()}, 0 );
	...
	TADkit.service('loading', function() {
		return {
			getResult: function() {
	...
	TADkit.service('ini', [ 'ConvertColors', function (ConvertColors) {
		return {
			parse: function(data) {
				var regex = {
					section: /^\s*\[\s*([^\]]*)\s*\]\s*$/,
				...
				lines.forEach(function(line){
					...
					if(regex.param.test(line)){
						...
						if(section){
							// 1. Check if not already HEX
							// 2. Convert X11 name to CSS (RGB)
							var colorRGB = ConvertColors.nameToRGB( match[2] );
							value[section][match[1]] = colorRGB;
						}else{
