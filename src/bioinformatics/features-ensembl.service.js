(function() {
	...
	function FeaturesEnsembl(ONLINE, $log, $q, $http) {
		...
		return {
			 ...
			 * Use supplied address within overlay format.
			 *
			 * @param {Array} overlay Array of URL path separators.
			 * @param {Object} address Object containing URL path.
			 *
			 * @return {boolean} Online status.
			 */
			load: function(overlay, address) {
				overlay = overlay || ["http://rest.ensemblgenomes.org/overlap/region/","species","/","chrom",":","chromStart","-","chromEnd","?feature=gene;content-type=application/json"];
				...
				var self = this;
				if (ONLINE) {
					dataUrl = overlay.object.url[0] + address.speciesUrl + overlay.object.url[2] + address.chrom + overlay.object.url[4] + address.chromStart + overlay.object.url[6] + address.chromEnd + overlay.object.url[8];
				} else {
