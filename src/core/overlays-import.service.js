(function() {
	...
	/**
	 * @ngdoc service
	 * @name TADkit.service:OverlaysImport
	 * @description Import Overlays to Projects.
	 *
	...
	function OverlaysImport($log, $q, $http, uuid4) {
		...
		return {
			import: function(data, rows, cols) {
				...
				// Filter data if rows and colums are given
				var filteredData;
				if (rows.length > 0 && cols.length > 0) {
					filteredData = self.filter(parsedData, rows, cols);
				} else {
